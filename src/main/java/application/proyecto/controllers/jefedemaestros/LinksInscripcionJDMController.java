...
public class LinksInscripcionJDMController extends BaseController {

    @FXML private Label lblLinksAbiertos;
    @FXML private Label lblLinksCerrados;
    @FXML private Label lblLinksVencidos;
    @FXML private Label lblTotalInscritos;
    @FXML private Label lblClasesSinLink;
    ...
    @FXML private ComboBox<String> cbMaestroFiltro;
    @FXML private ComboBox<String> cbGrupoFiltro;
    ...
    @FXML private TableColumn<LinkInscripcionItem, String> colMaestro;
    @FXML private TableColumn<LinkInscripcionItem, String> colCreadoPor;
    @FXML private TableColumn<LinkInscripcionItem, String> colOrigen;
    ...
    @FXML private Label lblDetalleMaestro;
    ...
    private void configurarTabla() {
        ...
        colCreadoPor.setCellValueFactory(new PropertyValueFactory<>("maestroCreador"));
    ...
    private void configurarFiltrosBase() {
        ...
        cbMaestroFiltro.setItems(FXCollections.observableArrayList("todos"));
        cbGrupoFiltro.setItems(FXCollections.observableArrayList("todos"));
        ...
        cbMaestroFiltro.setValue("todos");
        cbGrupoFiltro.setValue("todos");
    ...
    private void configurarEventos() {
        ...
        cbMaestroFiltro.valueProperty().addListener((obs, oldValue, newValue) -> aplicarFiltro());
        cbGrupoFiltro.valueProperty().addListener((obs, oldValue, newValue) -> aplicarFiltro());
    ...
    private void cargarClases() {
        ...
        String sql = """
                select
                c.id_carga,
                concat(m.clave,' - ',m.nombre,' | ',g.nombre,' - ',ct.nombre,' - ',ce.nombre,' | ',
                ma.nombre,' ',ma.apellido_paterno,' ',ma.apellido_materno) as clase
                from carga c
    ...
    private void cargarLinks() {
        ...
        String sql = """
                ...
                actualizado_en,
                cerrado_en
                from vw_links_inscripcion_jefe
    ...
    private void cargarValoresFiltros() {
        ...
        String maestroActual = cbMaestroFiltro.getValue();
        String grupoActual = cbGrupoFiltro.getValue();

        ObservableList<String> ciclos = FXCollections.observableArrayList("todos");
        ObservableList<String> maestros = FXCollections.observableArrayList("todos");
        ObservableList<String> grupos = FXCollections.observableArrayList("todos");
        ...
        for (LinkInscripcionItem item : listaLinks) {
            ...
            if (!maestros.contains(item.getMaestro())) {
                maestros.add(item.getMaestro());
            }

            if (!grupos.contains(item.getGrupo())) {
                grupos.add(item.getGrupo());
            }
        }

        cbCicloFiltro.setItems(ciclos);
        cbMaestroFiltro.setItems(maestros);
        cbGrupoFiltro.setItems(grupos);

        cbCicloFiltro.setValue(ciclos.contains(cicloActual) ? cicloActual : "todos");
        cbMaestroFiltro.setValue(maestros.contains(maestroActual) ? maestroActual : "todos");
        cbGrupoFiltro.setValue(grupos.contains(grupoActual) ? grupoActual : "todos");
    }
    ...
    private void cargarMetricas() {
        String sqlLinks = """
            ...
            coalesce(sum(case when v.estado_formulario='vencido' then 1 else 0 end),0) as vencidos,
            coalesce((
            select count(distinct ac.id_carga,ac.id_alumno)
            ...
            from vw_links_inscripcion_jefe v
            """;

        String sqlClasesSinLink = """
            select count(*) as clases_sin_link
            from carga c
            left join link_inscripcion_clase l on c.id_carga=l.id_carga
            and l.id_estatus_general=1
            where c.id_estatus_general=1
            and l.id_link_inscripcion is null
            """;
        ...
        try (Connection con = ConexionBD.conectar()) {
            try (PreparedStatement ps = con.prepareStatement(sqlLinks);
                ...
                if (rs.next()) {
                    ...
                    lblLinksVencidos.setText(String.valueOf(rs.getInt("vencidos")));
                    lblTotalInscritos.setText(String.valueOf(rs.getInt("total_inscritos")));
                }
            }

            try (PreparedStatement ps = con.prepareStatement(sqlClasesSinLink);
                 ResultSet rs = ps.executeQuery()) {

                if (rs.next()) {
                    lblClasesSinLink.setText(String.valueOf(rs.getInt("clases_sin_link")));
                }
            }
    ...
    private void aplicarFiltro() {
        ...
        String maestro = valorCombo(cbMaestroFiltro).toLowerCase();
        String grupo = valorCombo(cbGrupoFiltro).toLowerCase();
        ...
        listaFiltrada.setPredicate(item -> {
            boolean coincideTexto =
                    texto.isEmpty() ||
                            ...
                            item.getMaestro().toLowerCase().contains(texto) ||
                            item.getMaestroCreador().toLowerCase().contains(texto) ||
            ...
            boolean coincideMaestro = maestro.equals("todos") || item.getMaestro().toLowerCase().equals(maestro);
            boolean coincideGrupo = grupo.equals("todos") || item.getGrupo().toLowerCase().equals(grupo);
            ...
            return coincideTexto && coincideCiclo && coincideMaestro && coincideGrupo && coincideEstado && coincideOrigen;
        });
    }
    ...
    private void handleGenerarLink() {
        ...
        if (getIdMaestroActual() == 0) {
            mostrarError("no hay maestro creador en sesion");
            return;
        }
    ...
    private void mostrarDetalle(LinkInscripcionItem item) {
        ...
        lblDetalleMaestro.setText(item.getMaestro());
